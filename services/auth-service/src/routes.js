...
router.post('/users', apiLimiter, authMiddleware, requireRole(ROLES.ADMIN), async (req, res) => {
  try {
...
router.get('/users', apiLimiter, authMiddleware, requireRole(ROLES.ADMIN, ROLES.SUPERVISOR), async (req, res) => {
  try {
